...
while True:
    ...
    if menu == "Play" or menu.strip().lower() == "play":
        ...
        break
    # ==========================================================
        # --- CHAPTER 2: THE WILDS OF AURELIA ---
        # ==========================================================
        if not game_over:
            print("\n=======================================")
            print("      CHAPTER 2: THE GOLDEN REALM      ")
            print("=======================================")
            print("The dark stone walls of Blackrun finally disappear behind the horizon.")
            print("The dirt path brings you straight into the vast, wild borders of the Aurelia Kingdom.")
            print("Suddenly, rustling from the nearby bushes echoes through the trees...")

            # Dynamic Encounter Selection
            encounters = ["Wolf", "Bear", "Bandits"]
            wild_spawn = random.choice(encounters)

            if wild_spawn == "Wolf":
                print("\n[🐺] A fierce Timber Wolf leaps onto the path, teeth bared!")
                enemy_name = "Timber Wolf"
                enemy_hp = 25
                enemy_damage = 5
            elif wild_spawn == "Bear":
                print("\n[🐻] A hulking Grizzly Bear emerges from the thicket, roaring fiercely!")
                enemy_name = "Grizzly Bear"
                enemy_hp = 40
                enemy_damage = 8
            else:
                print("\n[⚔️] Two Aurelian Highway Bandits jump out from behind a boulder!")
                enemy_name = "Highway Bandits"
                enemy_hp = 32
                enemy_damage = 6

            # --- CHAPTER 2 COMBAT LOOP ---
            while enemy_hp > 0 and player_hp > 0:
                print(f"\nYour HP: {player_hp} | {enemy_name} HP: {enemy_hp} | Potions: {potions_count}")
                battle_choice = input("1. Attack | 2. Magic | 3. Potion | 5. Check Stats: ")
                ...
                if battle_choice == "1":
                    if not inventory_weapons:
                        ...
                        if weap_sel.isdigit() and 1 <= int(weap_sel) <= len(inventory_weapons):
                            ...
                            # Scaling damage based on skill level
                            base_dmg = 10
                            if weapon in ["Iron Sword", "Iron Dagger", "Iron Spear"]:
                                dmg = base_dmg + (one_handed_level // 5)
                                one_handed_level += 1
                                print(f"[+] Hit with {weapon} for {dmg} damage! One-Handed up to Lv {one_handed_level}!")
                            elif weapon == "Steel Axe":
                                dmg = base_dmg + 4 + (two_handed_level // 5)
                                two_handed_level += 1
                                print(f"[🪓] Hit with {weapon} for {dmg} damage! Two-Handed up to Lv {two_handed_level}!")
                            elif weapon == "Hunting Bow":
                                dmg = base_dmg + 2 + (archery_level // 5)
                                archery_level += 1
                                print(f"[🏹] Hit with {weapon} for {dmg} damage! Archery up to Lv {archery_level}!")
                            
                            enemy_hp -= dmg
                        else:
                            print("[-] Invalid selection! You fumbled your strike.")
                    ...
                    if player_mana >= 10:
                        ...
                        if spell_choice == "1":
                            destruction_level += 1
                            dmg = 12 + (destruction_level // 5)
                            enemy_hp -= dmg
                            print(f"[🔥] Fire Strike deals {dmg} damage! Destruction up to Lv {destruction_level}!")
                        elif spell_choice == "2":
                            guerison_level += 1
                            heal_val = 20 + (guerison_level // 5)
                            player_hp = min(100, player_hp + heal_val)
                            print(f"[✨] Fast Heal restores +{heal_val} HP! Guérison up to Lv {guerison_level}!")
                        else:
                            print("[-] Invalid spell choice! Mana lost.")
                    else:
                        print("[-] Out of Mana!")
                    ...
                    print(f"\n--- {name}'s UPDATED STATS ---")
                    print(f"Health: {player_hp}/100 | Mana: {player_mana}/100")
                    print(f"One-Handed: Lv {one_handed_level} | Two-Handed: Lv {two_handed_level} | Archery: Lv {archery_level}")
                    print(f"Destruction: Lv {destruction_level} | Guérison: Lv {guerison_level}")
                    print("-------------------")
                    continue

                if enemy_hp > 0:
                    # Apply player defense reduction
                    actual_dmg = max(1, enemy_damage - int(defense))
                    player_hp -= actual_dmg
                    print(f"[-] The {enemy_name} hits you back! (-{actual_dmg} HP)")
                
                if player_hp <= 0:
                    game_over = True

            # --- CHAPTER 2 VICTORY & ARRIVAL ---
            if not game_over:
                print(f"\n[🎉] You defeated the wild threat of the {enemy_name}!")
                if enemy_name == "Highway Bandits":
                    player_gold += 80
                    print("[💰] You looted the bandits' pockets and found 80 Gold!")
                    potions_count += 1
                    print("[🧪] Found 1x Heal Potion!")
                else:
                    print(f"You harvest rare crafting resources from the fallen beast.")
                    player_gold += 30 

                print("\nWith the roads secure, the trees break to reveal massive, shining white walls.")
                print("Magnificent golden flags flutter proudly in the wind.")
                print("Welcome to the Citadel Gates of the Aurelia Kingdom. Your destiny awaits!")
                
                # --- MEETING ALVOR ---
                print("\n=======================================")
                print("       THE CITADEL OF AURELIA          ")
                print("=======================================")
                print("You approach the massive iron-reinforced gates of the Citadel.")
                print("A heavily armored dwarf with a thick braided beard smithing near the barracks calls out to you.")
                print('"Hold there, traveler! You look like you\'ve been through hell and back," he grunts.')
                print('"The name\'s Alvor, Master Blacksmith of the Vanguard."')

                print('\nAlvor looks at your gear and sighs. "Can\'t have a hero wandering Aurelia looking like a peasant."')
                print('"Here, take some proper gear from my personal forge."')

                # Alvor rewards you
                inventory_weapons.append("Steel Vanguard Blade")
                player_gold += 100
                print("[⚔️] Added 'Steel Vanguard Blade' to your inventory weapons!")
                print("[💰] Alvor slips you 100 Gold for pocket change!")

                print('\nAlvor leans in closer, his expression growing serious.')
                print('"Listen, since I helped you out, I need a favor. Goblins have infested the Whispering Mines to the north."')
                print('"They stole my master smithing hammer. Bring it back to me, and I\'ll forge you something legendary."')
                print(f'"{name}, do you accept this quest?"')

                while True:
                    quest_choice = input("Accept Quest? (yes/no): ").lower().strip()
                    if quest_choice in ["yes", "y"]:
                        print('\n[📜] QUEST ACCEPTED: The Blacksmith\'s Missing Hammer')
                        break
                    elif quest_choice in ["no", "n"]:
                        print('\n[📜] QUEST FORCED: The Blacksmith\'s Missing Hammer')
                        print('"Ha! You don\'t really have a choice if you want passage through my gates, lad," Alvor chuckles.')
                        break
                    else:
                        print("[-] Please type 'yes' or 'no'.")

                # --- AUTOMATIC SAVE FILE SYSTEM ---
                print("\n=======================================")
                print("         💾 SYSTEM AUTO-SAVE           ")
                print("=======================================")
                print("Writing game save state to file...")
                
                try:
                    with open("save_file.txt", "w") as save_file:
                        save_file.write(f"Name:{name}\n")
                        save_file.write(f"HP:{player_hp}\n")
                        save_file.write(f"Mana:{player_mana}\n")
                        save_file.write(f"Gold:{player_gold}\n")
                        save_file.write(f"OneHanded:{one_handed_level}\n")
                        save_file.write(f"TwoHanded:{two_handed_level}\n")
                        save_file.write(f"Archery:{archery_level}\n")
                        save_file.write(f"Destruction:{destruction_level}\n")
                        save_file.write(f"Guerison:{guerison_level}\n")
                        save_file.write(f"Weapons:{','.join(inventory_weapons)}\n")
                    print("[🎉] PROGRESS SUCCESSFULLY WRITTEN TO 'save_file.txt'!")
                except Exception as e:
                    print(f"[-] Auto-save storage failed: {e}")
                
                print("=======================================")
                print("\n=== TO BE CONTINUED IN CHAPTER 3: THE WHISPERING MINES ===")
                break
